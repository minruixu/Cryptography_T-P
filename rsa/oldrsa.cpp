...
msg multiply(msg a, msg b,msg _n){
    // 计算 a * b
    ...
    if(raw[2047]%2) result[2047] = 1;
    return mod(result,_n);
}
msg pow_n(msg n,msg k, msg _n){
    msg r = 0x01;
    while(k.any()){
        cout << k <<endl;
        if(k[0]) r = multiply(r,n,_n);
        n = multiply(n,n,_n);
        k = k>>1;
    }
    return r;
...
//        pmsg r = random();
    ...
    string decodeOAEP(msg ciphertext){
        ...
        pmsg r = hash(P1) ^ P2;
        cout << "decode(r): "<< r.to_string() << endl;
        pmsg M = hash(r) ^ P1;
...
//     RSA 公钥加密
    msg RSAencode(msg encode_plain){
        ...
        msg cipher = pow_n(encode_plain,Wint2msg(_pubkey),Wint2msg(_n));
        return cipher;
    }
    // RSA 私钥解密
    msg RSAdecode(msg encode_cipher){
        // 输入2048比特的msg，进行次方运算，进行解密
        msg plain = pow_n(encode_cipher,Wint2msg(_prikey),Wint2msg(_n));
        return plain;
    }
    ...
    string test(string input){
        ...
        cout << "encode inter:" << inter <<endl;
        cout << "+"<<endl;
        inter = RSAdecode(inter);
        cout << "decode inter:" << inter <<endl;
        cout << "+"<<endl;
...
//    cout << msg2Wint(multiply(Wint2msg(Wint(p)),Wint2msg(Wint(q))))<<endl;
